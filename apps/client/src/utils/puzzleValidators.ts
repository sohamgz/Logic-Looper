...
export const validateMatrixPuzzle = (puzzle: Puzzle, userGrid: number[][]): boolean => {
  if (puzzle.type !== 'matrix') return false;
...
export const validatePatternPuzzle = (puzzle: Puzzle, userAnswer: string): boolean => {
  if (puzzle.type !== 'pattern') return false;
  return userAnswer === puzzle.correctAnswer;
};

export const validateSequencePuzzle = (puzzle: Puzzle, userAnswers: number[]): boolean => {
  if (puzzle.type !== 'sequence') return false;
  
  const { solution } = puzzle;
  
  if (userAnswers.length !== solution.length) return false;
  
  return userAnswers.every((ans, i) => ans === solution[i]);
};

export const validateDeductionPuzzle = (puzzle: Puzzle, userSolution: Record<string, string>): boolean => {
  if (puzzle.type !== 'deduction') return false;
...
export const validateBinaryPuzzle = (puzzle: Puzzle, userAnswers: boolean[]): boolean => {
  if (puzzle.type !== 'binary') return false;
...
// Universal validator
export const validatePuzzle = (puzzle: Puzzle, userAnswer: any): boolean => {
  switch (puzzle.type) {
    case 'matrix':
