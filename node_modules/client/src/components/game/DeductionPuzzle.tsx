...
export const DeductionPuzzle = ({ clues, grid, solution, onSubmit }: Props) => {
  const [userSolution, setUserSolution] = useState<Record<string, string>>(
    grid.rows.reduce((acc, person) => ({ ...acc, [person]: '' }), {})
  );

  const handleSelect = (person: string, color: string) => {
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                {grid.cols.map((color) => (
                  <button
                    key={color}
                    onClick={() => handleSelect(person, color)}
                    className={`px-4 py-2 rounded-lg border-2 transition-all ${
                      userSolution[person] === color
