...
import { Puzzle, PuzzleType } from '../types/puzzle.types';
...
export const generateSeedFromDate = (date: string): number => {
  const hash = CryptoJS.SHA256(date).toString();
  return parseInt(hash.substring(0, 8), 16);
};

// Get puzzle type for a given date (rotates through 5 types)
export const getPuzzleTypeForDate = (date: string): PuzzleType => {
  const types: PuzzleType[] = ['matrix', 'pattern', 'sequence', 'deduction', 'binary'];
  const dayOfYear = dayjs(date).dayOfYear();
  return types[dayOfYear % 5];
};

// Generate Matrix Puzzle (Sudoku-like 4x4)
const generateMatrixPuzzle = (seed: number, date: string): Puzzle => {
  ...
  const grid: number[][] = finalSolution.map(row => [...row]);
...
const generatePatternPuzzle = (seed: number, date: string): Puzzle => {
  const rng = new SeededRandom(seed);
...
const generateSequencePuzzle = (seed: number, date: string): Puzzle => {
  const rng = new SeededRandom(seed);
...
const generateDeductionPuzzle = (seed: number, date: string): Puzzle => {
  const rng = new SeededRandom(seed);
...
const generateBinaryPuzzle = (seed: number, date: string): Puzzle => {
  const rng = new SeededRandom(seed);
...
export const generateDailyPuzzle = (date: string): Puzzle => {
