...
export const GameScreen = () => {
    ...
    const { currentPuzzle, gameState, isLoading } = useSelector((state) => state.puzzle);
    ...
    if (gameState.isComplete) {
        return (<div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-green-50 to-green-100">
        <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className="card text-center max-w-md">
    ...
    return (<div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-4xl mx-auto px-4">
        ...
        {/* Puzzle Content */}
        <div className="card">
          {currentPuzzle.type === 'matrix' && (<MatrixPuzzle grid={currentPuzzle.grid} solution={currentPuzzle.solution} onSubmit={handleSubmit}/>)}

          {currentPuzzle.type === 'pattern' && (<PatternPuzzle sequence={currentPuzzle.sequence} options={currentPuzzle.options} onSubmit={handleSubmit}/>)}

          {currentPuzzle.type === 'sequence' && (<SequencePuzzle numbers={currentPuzzle.numbers} solution={currentPuzzle.solution} onSubmit={handleSubmit}/>)}

          {currentPuzzle.type === 'deduction' && (<DeductionPuzzle clues={currentPuzzle.clues} grid={currentPuzzle.grid} solution={currentPuzzle.solution} onSubmit={handleSubmit}/>)}

          {currentPuzzle.type === 'binary' && (<BinaryPuzzle gates={currentPuzzle.gates} onSubmit={handleSubmit}/>)}
        </div>

        {/* Hint Button */}
        <div className="mt-6 text-center">
          <button onClick={handleUseHint} disabled={gameState.hintsUsed >= currentPuzzle.maxHints} className="bg-yellow-500 text-white px-6 py-2 rounded-lg hover:bg-yellow-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
